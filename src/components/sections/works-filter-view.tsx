...
export function WorksFilterView({ works }: WorksFilterViewProps) {
  // Save current scroll position and target slug to sessionStorage
  ...
  // Continuously record scroll position on /works
  useEffect(() => {
    let timer: number;
    const onScroll = () => {
  ...
  // Multi-phase scroll restoration when returning from a work detail page
  useEffect(() => {
  ...
  const live = works.filter((w) => w.status === "已上线");
  const incubating = works.filter((w) => w.status === "在研");

  return (
    <div id="works-explorer" className="works-explorer space-y-16">
      {live.length > 0 && (
